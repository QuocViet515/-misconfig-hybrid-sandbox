...
- Theo policy mặc định, chỉ finding `auto_remediate` mới được execute tự động.
- Nếu muốn chạy một finding `manual_review` sau khi đã được approver chấp thuận, truyền rõ `--approve-finding-id <finding_id>`.
- Flow hiện hỗ trợ:
...
./.venv/bin/python -m src.remediation.aws_runtime_executor \
  ...
  --region ap-southeast-1 \
  --project-prefix threat-demo \
  --approve-finding-id <manual_finding_id> \
  --execute \
  --force-rds-cutover \
  --delete-archived-rds
```
